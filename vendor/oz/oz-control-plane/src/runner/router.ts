...
import { ConfigError, ProviderError, RateLimitError, isRateLimitError } from "./errors.js"
...
function loadCandidate(providerKey: string, fallbackType?: ProviderType): ProviderCandidate | null {
  ...
  const apiKey = env(providerEnvName(providerKey, "API_KEY")) || env("OZ_PROVIDER_API_KEY")

  const baseUrl = env(providerEnvName(providerKey, "BASE_URL")) || env("OZ_PROVIDER_BASE_URL")
  ...
  return { providerKey, type, model, apiKey, baseUrl, windowSeconds, messagesLimit }
}
...
function parseFallbackOrder(harness: string, primaryProviderKey: string): string[] {
  ...
  const list = (specific || "").split(",").map((s) => s.trim()).filter(Boolean)
  const out: string[] = []
  for (const k of list) {
...
export function queueEnabled(): boolean {
  const v = (env("OZ_QUEUE_ON_SATURATION") || "").toLowerCase()
  return v === "1" || v === "true"
}
