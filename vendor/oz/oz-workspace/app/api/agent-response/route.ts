    ...
    if (roomId && agentId) {
      ...
      try {
        const marker = await prisma.agentCallback.findUnique({
          where: { id: `oz-run:${taskId}` },
          select: { response: true },
        })
        const runId = marker?.response
        if (runId) {
          after(async () => {
            for (let attempt = 0; attempt < 6; attempt++) {
              try {
                const status = await getTaskStatus(runId, userIdForInvocations)
                const artifacts = status.artifacts ?? []
                if (artifacts.length > 0) {
                  await saveArtifacts(artifacts, { roomId, agentId, userId: userIdForInvocations })
                  break
                }
