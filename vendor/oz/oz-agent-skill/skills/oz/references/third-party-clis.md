...
The `-agents` tagged variants of compatible images can include Claude Code, Codex, and Gemini CLI preinstalled. Base tags (without `-agents`) do not include coding agent CLIs.

## Reporting Artifacts

When a third-party CLI creates a PR or branch, Oz does not automatically detect it. You must explicitly register these artifacts so they appear in the workspace UI.
...
```sh
oz secret create ANTHROPIC_API_KEY --team --value-file key.txt --description "Anthropic API key for Claude Code"
```
...
```sh
oz secret create OPENAI_API_KEY --team --value-file key.txt --description "OpenAI API key for Codex CLI"
```
...
```sh
oz secret create GEMINI_API_KEY --team --value-file key.txt --description "Gemini API key for Gemini CLI"
```
...
```sh
oz secret create AMP_API_KEY --team --value-file key.txt --description "Amp API key"
```
...
```sh
oz secret create GH_TOKEN --team --value-file token.txt --description "GitHub PAT with Copilot Requests permission"
```
